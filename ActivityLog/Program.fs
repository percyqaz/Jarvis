...
let LOGFILE = "activity_log.txt"

while not (File.Exists LOGFILE) do
...
let postFix2 (sep: string) (input: string) =
...
let postFix (sep: string) (expectedSuffix: string) (input: string) =
...
for s in log do
    ...
    if date.Date = timestamp.Date then
        let a = identify title
        add_activity a
        insert_color timestamp a
print_breakdown()
render_image("activity-"+date.ToString("dd-MM-yy")+".png")